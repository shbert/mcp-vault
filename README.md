...
> `tenant_id` et `expected_aud` dans `secret_wrap` alimentent le binding C18 complet côté `secret_consume` *(v0.6.5)*.
...
| Version              | Approche                                                                                                            |
| -------------------- | ------------------------------------------------------------------------------------------------------------------- |
| **v0.6.5** (actuel)  | Clés sur S3 chiffrées AES-256-GCM+AAD, mémoire seule au runtime — hardening C18 : singleton JWT + binding complet tenant_id/aud |
...
├── VERSION                   # 0.6.5
├── DESIGN/mcp-vault/
│   ├── ARCHITECTURE.md       # Spécification détaillée (v0.6.5)
│   ├── TECHNICAL.md          # Documentation technique (v0.6.5)
...
**Licence** : Apache 2.0 | **Auteur** : Cloud Temple | **Version** : 0.6.5
